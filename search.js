/*The below is a summaries Array of object 
to be fetched from API Data*/
let summaries = [
    {
        "id": 0,
...
/* 
   Below function will transform an array to an object ( HashMap )
   as a key-Value pair 

   using the reducing on array the key as an id 
...
/* This is Transformed Object*/
const summaryObject = arrayToObject(summaries, "id");


/* 
   Below function will search  into an object for 
   user provided string, 
*/
function search(wordToMatch, summaryObject, k) {

    const regex = new RegExp(wordToMatch, 'gi');

    let filterArray = [];
    /*
       if String is matched with any value of object's summary-->value
       it will pull the object out 
       and push into a new Array and the return the 
       "k"count of outcomes
    */
    for (let summary in summaryObject) {
        if (summaryObject[summary].summary.match(regex)) {
            filterArray.push(summaryObject[summary])
        }
    }
    return filterArray.splice(0, k);

}

/* A sample test check */

/********* un-comment this line to view

var output = search("\u00a0What",summaryObject,4);
